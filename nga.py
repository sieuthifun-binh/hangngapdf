...
# --- CẤU HÌNH API AI (Sửa lỗi NotFound) ---
try:
...
def split_pdf_advanced(uploaded_file, mode, custom_pages=None):
    ...
    if mode == 'Trang chẵn':
        ...
        pages_to_extract = [i for i in range(total_pages) if (i + 1) % 2 != 0]
    elif mode == 'Tùy chọn' and custom_pages:
...
def ai_process_table(uploaded_file):
    ...
    response = model.generate_content([
        "Hãy trích xuất bảng trong ảnh này thành định dạng JSON chuẩn. Chỉ trả về JSON.",
...
# --- GIAO DIỆN ---
tab1, tab2, tab3 = st.tabs(["✂️ Băm PDF", "📝 PDF sang Word", "📊 PDF Excel (AI)"])

with tab1:
    mode = st.radio("Chế độ:", ["Trang chẵn", "Trang lẻ", "Tùy chọn"])
    pages = st.text_input("Trang (VD: 1,3,5):") if mode == 'Tùy chọn' else None
    uploaded = st.file_uploader("Tải PDF:", type="pdf")
    if st.button("Băm ngay"):
        if uploaded:
            data = split_pdf_advanced(uploaded, mode, pages)
            st.download_button("📥 Tải về", data, "split.pdf", "application/pdf")

with tab2:
    file_word = st.file_uploader("Tải PDF:", type="pdf")
...
with tab3:
    uploaded_scan = st.file_uploader("Tải PDF scan:", type="pdf")
